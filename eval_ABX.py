    ...
    for contextmode in contextmodes:
        ...
        if 'within' in speakermodes:
            ...
            if contextmode == "any":
                ...
                group_confusion = torch.sparse.sum(group_confusion, dimnwithin).to_dense()
            ...
            scores[f'within-{contextmode}'] = (phone_confusion.sum() /
                                (divisor_speaker > 0).sum()).item()
            print(f"...done. ABX {contextmode}_context within_speaker : {scores[f'within-{contextmode}']}")
        ...
        if 'across' in speakermodes:
            ...
            group_confusion = torch.sparse.sum(
                group_confusion, dimnacross).to_dense()
            group_confusion = reduce_sparse_data(group_confusion, divisor_context)
            S, p1, p2 = group_confusion.size()
            ...
            phone_confusion = reduce_sparse_data(group_confusion.sum(dim=0),
                                                 divisor_speaker)
            scores[f'across-{contextmode}'] = (phone_confusion.sum() /
                                (divisor_speaker > 0).sum()).item()
            print(f"...done. ABX {contextmode}_context across_speaker : {scores[f'across-{contextmode}']}")
...
def parse_args(argv):
    ...
    parser.add_argument('--speaker_mode', type=str, default='all',
                        choices=['all', 'within', 'across'],
                        help="Choose the speaker mode of the ABX score to compute")
    parser.add_argument('--context_mode', type=str, default='all', 
...
def main(argv):
    ...
    scores = ABX(feature_function, args.path_item_file,
                 ...
                 step_feature, speakermodes, contextmodes,
                 cuda=args.cuda,
